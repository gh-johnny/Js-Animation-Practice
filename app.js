...
let aboutNavBarTl = gsap
  .timeline({ defaults: { color: navLightColor } })
...
gsap.fromTo(
  '#stars3',
  { x: window.innerWidth },
  { x: -1500, ease: 'power1.out', duration: 85 }
)
//Flotating animation
...
slidePicker.on('click', function () {
  ...
  function slideOut(whereTo, n1, n2) {
    ...
    gsap
      .timeline()
      .to($('.slide')[$(prevPick).index()].children[n1], {
        x: `${whereTo}${window.innerWidth}`,
        duration: 1,
        ease: 'power4.out'
      })
    gsap
      .timeline()
      .to($('.slide')[$(prevPick).index()].children[n2], {
        x: `${whereTo}${window.innerWidth}`,
        duration: 1,
        ease: 'power4.out',
        delay: 0.25
  ...
  function slideIn(whereTo, c1, c2) {
    gsap
      .timeline({ defaults: { duration: 1, ease: 'bounce.out', x: 0 } })
      .fromTo(c1, { x: `${whereTo}${window.innerWidth}` }, { delay: 1.25 })
      .fromTo(c2, { x: `${whereTo}${window.innerWidth}` }, { delay: -0.25 })
    setTimeout(() => {
...
//Contact section
//Navigation bar colors animation
let contactNavBarTl = gsap
  .timeline({ defaults: { color: navDarkColor } })
  .to('.marker-projects', { color: navLightColor })
...
clipboardIcon.on('click', function () {
    const copyText = this.parentNode.firstElementChild
    navigator.clipboard.writeText(copyText.innerText);
  if (trackBtn == this) return
  if (
    $(clipboardIcon[0].firstElementChild).attr('class') ===
      'fa-solid fa-clipboard-check' ||
    $(clipboardIcon[1].firstElementChild).attr('class') ===
      'fa-solid fa-clipboard-check'
  ) {
    $(clipboardIcon[0].firstElementChild).toggleClass(
      'fa-clipboard fa-clipboard-check'
    )
    $(clipboardIcon[1].firstElementChild).toggleClass(
      'fa-clipboard fa-clipboard-check'
    )
    trackBtn = this
    return
  }
  $(this.firstElementChild).toggleClass('fa-clipboard fa-clipboard-check')
  trackBtn = this
})
